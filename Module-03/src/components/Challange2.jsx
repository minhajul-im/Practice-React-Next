...
export default function Challlange2() {
  ...
  return (
    <>
      <SearchBar query={query} onChangeInput={handleChange} />
      <hr />
      <List items={results} />
    </>
  );
...
function SearchBar({ query, onChangeInput }) {
  return (
    <label>
      Search: <input value={query} onChange={onChangeInput} />
    </label>
  );
}
